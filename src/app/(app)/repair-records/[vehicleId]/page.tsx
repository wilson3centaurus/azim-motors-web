...
export default async function VehicleHistoryPage({ params }: { params: Promise<{ vehicleId: string }> }) {
  ...
  const [{ data: vehicle }, { data: records }] = await Promise.all([
    supabase.from('vehicles').select('*, customers(full_name, phone)').eq('id', vehicleId).single(),
    supabase.from('repair_records').select(`
      *, job_cards(job_number, complaint, job_card_parts(quantity_used, unit_cost, parts(name)))
    `).eq('vehicle_id', vehicleId).order('completed_at', { ascending: false }),
  ])
  ...
  return (
    <div className="p-4 sm:p-6 max-w-3xl space-y-4 sm:space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-500">{vehicle.make} {vehicle.model} {vehicle.year} · Owner: {(vehicle as any).customers?.full_name}</p>
      </div>
          ...
          {records?.map((r: any) => (
            <div key={r.id} className="relative pl-12">
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-5">
                ...
                {r.job_cards?.job_card_parts?.length > 0 && (
                  <div className="mt-3 border-t border-slate-100 pt-3">
                    <p className="text-xs font-medium text-slate-500 mb-1">Parts Used</p>
                    <div className="flex flex-wrap gap-1.5">
                      {r.job_cards.job_card_parts.map((p: any, i: number) => (
